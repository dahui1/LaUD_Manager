    ...
    $.ajax({
        ...
        success: function (data) {
            $(data).each(function (i, value) {
                $("#nodesinfo").append("<tr id=\"" + value.endpoint + "\"><td>" + value.endpoint + "</td><td>" + value.status + "</td><td>" + value.load + "</td><td>" + value.owns + "</td><td>" + value.range + "</td><td> <button class=\"btn btn-default\" onclick=\"getDetails('" + value.endpoint + "')\">详细信息</button></td></tr>");
            });
        },
        error: function () {
            alert("服务器连接失败，请稍后尝试！");
...
function getDetails(endPoint) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function (data) {
            $("#detailedinfo").append("<tr id=\"" + data.endpoint + "\"><td>EndPoint</td><td>" + data.endpoint + "</td></tr><td>Token</td><td>" + data.range + "</td></tr><td>UpTime</td><td>" + data.uptime + "</td></tr><td>memMax</td><td>" + data.memMax + "</td></tr><td>memUsed</td><td>" + data.memUsed + "</td></tr><td>datacenter</td><td>" + data.dataCenter + "</td></tr><td>rack</td><td>" + data.rack + "</td></tr><td>status</td><td>" + data.status + "</td></tr><td>state</td><td>" + data.state + "</td></tr><td>load</td><td>" + data.load + "</td></tr><td>owns</td><td>" + data.owns + "</td></tr>");
        },
        error: function () {
            alert("服务器连接失败，请稍后尝试！");
            window.location = "../index.jsp";
